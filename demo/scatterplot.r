...
ggvis(mtcars, props(x = ~wt, y = ~wt/mpg)) + mark_symbol()
ggvis(mtcars, props(x = ~factor(cyl), y = ~mpg)) + mark_symbol()


# With colour
# continuous:
ggvis(mtcars, props(x = ~wt, y = ~mpg, fill = ~cyl)) + mark_symbol()
# and discrete:
ggvis(mtcars, props(x = ~wt, y = ~mpg, fill = ~factor(cyl))) + mark_symbol()
...
ggvis(mtcars, props(x = ~wt, y = ~mpg)) +
  layer(mark_symbol()) +
  layer(layer(mark_symbol(props(fill := "red", size := 25))))
...
ggvis(mtcars, props(x = ~wt, y = ~mpg)) +
  mark_symbol() +
  mark_symbol(props(fill := "red", y = prop(~ qsec, scale = "yq"))) +
...
ggvis(pressure, props(x = ~temperature, y = ~log(pressure))) + mark_symbol()
