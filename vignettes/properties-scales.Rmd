...
This introduces `props()`, a convenient function for creating property objects and binding them to property names. The next chapter shows you how to create the property objects by hand, giving you more control over the specification at the cost of some extra typing.
...
Those examples follow a common pattern: `=` is always used with a scalar, and `~` is always used with the name of the variable. What if you try the opposite?

* `props(x ~ 0)`: sets the x position to the value 0

* `props(x = wt)`: is an error, because 
...
------- ----|--------|-------------------------
constant    | yes    | Pixel location, or exact colour | `x = 1`
constant    | yes    | Annotation position in data space | `x ~ 1`
variable    | no     | Variable already contains visual values | `x ~ I(colour)`
variable    | no     | Map variable to property | `x ~ mpg`
