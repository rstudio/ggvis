...
mtcars %>% ggvis(~disp, ~wt) %>%
  layer_points() %>%
  set_dscale("x", "numeric", domain = c(50, 500), nice = FALSE) %>%
  set_dscale("y", "numeric", domain = c(0, 6), nice = FALSE)
```
...
```{r, eval = FALSE}
set_dscale(vis, "x", "numeric")
set_dscale(vis, "y", "numeric")
set_dscale(vis, "shape", "nominal")
```

You can also provide other arguments to the underlying scale:

```{r, eval = FALSE}
set_dscale(vis, "x", "numeric", domain = c(10, 100))
set_dscale(vis, "x", "numeric", trans = "log")
```
...
df %>% ggvis(~x, ~y, text := ~labels, font = ~labels, fontSize := 40) %>%
  layer_text() %>%
  add_scale(scale_ordinal("font", range = c("Helvetica Neue", "Times New Roman")))
```
...
mtcars %>% ggvis(y = ~mpg) %>%
  ...
  add_guide_axis("x", "xwt", orient = "bottom", offset = 20, properties =
...
df %>% 
  ...
  layer_points() %>%
  add_guide_legend(fill = "stroke", properties = list(legend = props(y := 50)))
