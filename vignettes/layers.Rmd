...
```{r}
mtcars %>% ggvis(x = ~wt, y = ~mpg, stroke := "red") %>% layer_point()
...
```{r}
df %>% ggvis(~x, ~y, height := prop(80, scale = "y")) %>% layer_area()

df <- data.frame(x = 1:10, y = (1:10) ^ 2)
df %>% ggvis(~x, ~y) %>%
  layer_ribbons(height = prop(input_slider(0, 100), scale = "y")) %>%
