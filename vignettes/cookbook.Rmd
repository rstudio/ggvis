...
The bin selection can be controled by specifying `binwidth` and at most one of `center` or 
`boundary` of one of the bins.  `boundary` and `center` may be outside the range of 
the data.

```{r, message = FALSE}
faithful %>% ggvis(~eruptions) %>% layer_histograms(binwidth=0.5, boundary=0)
faithful %>% ggvis(~eruptions) %>% layer_histograms(binwidth=0.5, center=0)
```
...
```{r, message = FALSE}
cocaine %>% ggvis(~month, fill := "#fff8dc") %>%
  layer_histograms(binwidth = 1, center = 0) %>%
  add_axis("x", title = "month") %>%
  add_axis("y", title = "count")
```
## Box plots
