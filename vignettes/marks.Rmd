...
ggvis() + 
  mark_symbol(data = mtcars, props(x = ~wt, y = ~mpg, stroke := "red"))
```
...
ggvis(hec, props(x = ~Hair, y = ~Eye, fill = ~Freq),
  mark_rect(props(width := band(), y := band())))
```
...
ggvis(df, props(x = ~x, y = ~y, stroke := "red"), mark_line())
...
* `layer_histogram` = `mark_rect` + `transform_bin`
* `layer_freqpoly` = `mark_line` + `transform_bin`
* `layer_smooth` = (`mark_line` + `mark_area`) + `transform_bin`
...
* `geom_freqpoly`
* `geom_hex`
* `geom_histogram` = `layer_histogram` = `mark_bar` + `transform_bin`
...
* `geom_rug`
* `geom_segment`
* `geom_smooth` = `layer_smooth` = `mark_area` + `mark_line` + `transform_smooth`
