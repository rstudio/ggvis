...
* `layer_area` = `geom_ribbon`, `geom_area`
* `layer_path` = `geom_path`, `geom_line`
* `layer_rect` = `geom_rect`, `geom_bar`, `geom_tile`
* `layer_point` = `geom_point`
* `layer_text` = `geom_text`

All marks start with `layer_` so that you can always use tab autocomplete to
remind yourself of the options.

There are two marks that have no equivalent in ggplot2:

* `layer_arc` which allows you to draw arcs. You could accomplish something
...
* `layer_image` allows you to place arbitrary images on a plot. There is
...
`layer_path` produces lines if fill is empty, and polygons if it's set to a
value.
...
In ggvis marks strictly represent drawing; a layer combines a mark with a
transformation, and often a set of default props. For example:

* `layer_histogram` = `layer_rect` + `transform_bin`
* `layer_freqpoly` = `layer_path` + `transform_bin`
* `layer_smooth` = (`layer_path` + `layer_area`) + `transform_bin`

All layers start with `layer_` and the source will typically be simple so is
worth reading:
