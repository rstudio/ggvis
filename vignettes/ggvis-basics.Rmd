...
As well as `input_slider()`, ggvis also provides `input_checkbox()`, `input_checkboxgroup()`, `input_numeric()`, `input_radiobuttons()`, `input_select()` and  `input_text()`. See the examples in the documentation for how you might use each one.
...
* `layer_smooth()` fits a smooth model to the data, and displays predictions
  ...
  in noisy data:
    ...
    qvis(mtcars, ~wt, ~mpg, layers = c("point", "smooth"), span = span)
    ```
