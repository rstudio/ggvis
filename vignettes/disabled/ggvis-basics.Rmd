...
Normally, we'll use the `%>%` (pronounced pipe) to feed the data into the first argument of `qvis()`. This isn't so important here, but will make more complicated graphics much easier to express:
...
mtcars %>% qvis(~wt)
# A barchart
mtcars %>% qvis(~factor(cyl))
```

You can add more variables to the plot by mapping them to other visual properties like `fill`, `stroke`, `size` and `shape`.

```{r}
mtcars %>% qvis(~wt, ~mpg, stroke = ~vs)
mtcars %>% qvis(~wt, ~mpg, fill = ~vs)
mtcars %>% qvis(~wt, ~mpg, size = ~vs)
mtcars %>% qvis(~wt, ~mpg, shape = ~factor(cyl))
```
...
```{r}
mtcars %>% qvis(~wt, ~mpg, fill := "red", stroke := "black")
mtcars %>% qvis(~wt, ~mpg, size := 300)
mtcars %>% qvis(~wt, ~mpg, shape := "cross")
```
...
mtcars %>% qvis(~wt, ~mpg, size := slider_s, opacity := slider_o)
```
...
mtcars %>% qvis(~wt, binwidth = binwidth)
```
...
mtcars %>% qvis(~wt, ~mpg, size := keys_s, opacity := 0.5)
```

You can also add on more complex types of interaction like tooltips:

```{r}
mtcars %>% qvis(~wt, ~mpg) %>% tooltip(function(df) df$wt)
```
...
The previous examples showed how to make scatterplots and barcharts. To create more advanced visualisations, you'll need to use `ggvis()`. `ggvis()` works almost identically to `qvis()` but it doesn't automatically layer on any data - you'll need to do that yourself.
...
* Points, `layer_points()`. This is the default layer when you use`qvis()`
...
* `layer_histograms()` and `layer_barcharts()` which allow you to explore the
  ...
  layer when `qvis()` is called with a dataset and an `x` variable.
    ...
    The most important parameter to `layer_histograms()` is the bin width:

    ```{r}
    mtcars %>% qvis(~wt, binwidth = 1)
    mtcars %>% qvis(~wt, binwidth = 0.1)
    ```
...
Rich graphics can be created by combining multiple layers on the same plot. This easier to do: just use multiple layer functions:

```{r}
mtcars %>% ggvis(~wt, ~mpg) %>% layer_smoooths() %>% layer_points()
```
