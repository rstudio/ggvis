...
mtcars %>%
  ggvis(~wt, ~mpg) %>%
  layer_smooths(span = input_slider(0.5, 1, 1)) %>%
  layer_points(size := input_slider(100, 1000))
```
...
mtcars %>% ggvis(~wt, ~mpg) %>%
  layer_points(size := input_slider(100, 1000)) %>% 
  layer_points(fill := "red", size := input_slider(100, 1000))
```
...
`server.r`, create a reactive ggvis object, and call `render_ggvis()`. The
first argument is the plot object, the second is the session, and the third is the plot id (needs to match with `ui.r`).

```{r, eval = FALSE}
mtcars %>% 
  ggvis(~disp, ~mpg, size := input$size) %>% 
  layer_points() %>%
  render_ggvis(session, "ggvis") %>%
  render_controls(session, "ggvis")
```
