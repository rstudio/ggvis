...
test_that("bin_vector preserves dates and times", {
  ...
  res <- bin_vector(NYtimes, binwidth = 120)
  ...
  res <- bin_vector(UTCtimes, binwidth = 120)
  expect_identical(sum(res$count_), length(UTCtimes))
  expect_identical(attr(UTCtimes, "tzone"), attr(res$x_, "tzone"))


  # Can set boundary
  res <- bin_vector(dates, binwidth = 30, boundary = as.Date("2013-06-01"), pad = FALSE)
  expect_identical(sum(res$count_), length(dates))
  ...
  # Can set center
  res <- bin_vector(dates, binwidth = 30, center=as.Date("2013-07-01"),
                    pad = FALSE)
  expect_identical(sum(res$count_), length(dates))
